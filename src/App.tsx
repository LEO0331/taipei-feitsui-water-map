...
type MonitoringTab = 'waterQuality' | 'riverWaterQuality' | 'riverEcology' | 'amphibianEcology' | 'pumpingStations' | 'stormMachinery' | 'sedimentationBasins' | 'reservoirSedimentationSurveys' | 'parkWaterSafety' | 'clearWaterQuality' | 'tapWaterBusiness' | 'carlsonTrophicStateIndex' | 'twcSupport' | 'hydromet' | 'operation' | 'combinedDashboard' | 'dataTable';
...
function MonitoringTabs({
  ...
  const categories: Array<{ id: NavigationCategory; label: string; tabs: Array<{ id: MonitoringTab; label: string }> }> = [
    ...
    { id: 'rivers', label: text(language, '河川與生態', 'Rivers & Ecology'), tabs: [{ id: 'riverWaterQuality', label: t.riverWaterQuality }, { id: 'riverEcology', label: text(language, '底棲生物', 'Benthic Species') }, { id: 'amphibianEcology', label: text(language, '兩棲類調查', 'Amphibian Survey') }] },
...
export default function App() {
  ...
  if (!summary || !riverSummary || !pumpingSummary || !sedimentationBasinSummary || !twcSupportSummary || !parkWaterSafetySummary || !clearWaterSummary || !tapWaterBusinessSummary || !ctsiSummary || !sedimentationSurveySummary || !stormMachinerySummary || !riverEcologySummary || !amphibianSummary) {
