...
import AmphibianEcologyPanel from './AmphibianEcologyPanel';
  ...
  return (
    <main>
      ...
      {activeTab === 'parkWaterSafety' && (
        <ParkWaterSafetyPanel records={parkWaterSafetyRecords} summary={parkWaterSafetySummary} language={language} />
      )}
      ...
      {activeTab === 'operation' && (
        <>
          <section className="filters hydromet-filters">
            ...
            <label><span>{text(language, '雨量類別', 'Rainfall category')}</span><select value={operationRainfallCategory} onChange={(event) => setOperationRainfallCategory(event.target.value)}><option value="">{t.all}</option><option value="none">{text(language, '無雨', 'None')}</option><option value="light">{text(language, '小雨', 'Light')}</option><option value="moderate">{text(language, '中雨', 'Moderate')}</option><option value="heavy">{text(language, '大雨', 'Heavy')}</option><option value="unknown">{text(language, '未知', 'Unknown')}</option></select></label>
            <label><span>{text(language, '淨蓄水變化方向', 'Net storage direction')}</span><select value={operationNetDirection} onChange={(event) => setOperationNetDirection(event.target.value)}><option value="">{t.all}</option><option value="positive">{text(language, '增加', 'Positive')}</option><option value="negative">{text(language, '減少', 'Negative')}</option><option value="neutral">{text(language, '持平', 'Neutral')}</option><option value="unknown">{text(language, '未知', 'Unknown')}</option></select></label>
            <label><span>{text(language, '水位類別', 'Water-level category')}</span><select value={operationWaterLevelCategory} onChange={(event) => setOperationWaterLevelCategory(event.target.value)}><option value="">{t.all}</option><option value="low">{text(language, '低', 'Low')}</option><option value="normal">{text(language, '一般', 'Normal')}</option><option value="high">{text(language, '高', 'High')}</option><option value="unknown">{text(language, '未知', 'Unknown')}</option></select></label>
            ...
            <label><span>{text(language, '缺失值', 'Missing values')}</span><input type="checkbox" checked={operationMissingOnly} onChange={(event) => setOperationMissingOnly(event.target.checked)} /></label>
            <label>
              <span>{t.startDate}</span>
              <input
